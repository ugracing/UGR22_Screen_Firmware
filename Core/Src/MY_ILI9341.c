...
void ILI9341_SendData_Multi(uint8_t* Colordata, uint32_t size)
{
	...
	HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_RESET);
	//Write byte using SPI
	HAL_SPI_Transmit(&lcdSPIhandle, Colordata, size, 1000);
...
void write_font_char(int start_x, int start_y, int width, int height, const unsigned char * bit_array, int red, int green, int blue ) {
    ...
    // Set Window size
	ILI9341_SetCursorPosition(start_x, start_y, start_x + width - 1, start_y + height - 1);
//    Write_Command(0x2A);
...
//    Write_Command(0x2C); // Start pixel data
    ...
    for (int i=0; i<((width*height)/8); i++) {
    	...
    	for(int b=0; b<8; b++){
			if(bits & 0x80){
				ILI9341_SendData(cHigh);
				ILI9341_SendData(cLow);
			} else {
...
void write_GFXfont_char(int start_x, int start_y, int width, int height, const unsigned char * bit_array, int red, int green, int blue ) {
